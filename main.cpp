...
#include "../include/igl/include/igl/readSTL.h"
...
template <typename T>
Eigen::Matrix< T, Eigen::Dynamic, 3, Eigen::RowMajor> ConvertToEigenMatrix(std::vector<std::vector<T>> data)
{
  // Adapted from: https://stackoverflow.com/a/40512883
  Eigen::Matrix< T, Eigen::Dynamic, 3, Eigen::RowMajor> eMatrix(data.size(), data[0].size());
  for (int i = 0; i < data.size(); ++i)
    eMatrix.row(i) = Eigen::Matrix<T, 3, 1>::Map(&data[i][0], data[0].size());
  return eMatrix;
}
...
Usage:
  ...
  // Adapted from: TA Darren ()
  // test switching vao
  GLuint myVAO;
  std::vector<std::vector<float> > myV;
  std::vector<std::vector<GLuint> > myF;
  std::vector<std::vector<float> > myN;

  // You'll need to grab the code and include this
  igl::readSTL("../src/skull.stl", myV, myF, myN);
  Eigen::Matrix< float, Eigen::Dynamic, 3, Eigen::RowMajor> myEigV(ConvertToEigenMatrix(myV));
  Eigen::Matrix<GLuint, Eigen::Dynamic, 3, Eigen::RowMajor> myEigF(ConvertToEigenMatrix(myF));

  mesh_to_vao(myEigV, myEigF, myVAO);
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    glUniform1f(glGetUniformLocation(prog_id, "width_window"), width_window);
    glUniform1f(glGetUniformLocation(prog_id, "height_window"), height_window);
    ...
    for (int i = 0; i < 3; i++)
    {
      ...
      glUniform1i(glGetUniformLocation(prog_id, "is_boom"), i == 2);
      // moon left to be a skull
      if (animation_seconds > 20.0 && i == 1){
        glBindVertexArray(myVAO);
        glDrawElements(GL_PATCHES, myEigF.size(), GL_UNSIGNED_INT, 0);
      }
      else {
